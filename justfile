...
build-linux $SCONS_CACHE="build-linux":
  scons {{compile_commands}} -j4 gdextension_dir={{api_dir}} platform=linux

build-android-arm32 $SCONS_CACHE="build-android-arm32":
  scons {{compile_commands}} -j4 gdextension_dir={{api_dir}} platform=android arch=arm32
build-android-arm64 $SCONS_CACHE="build-android-arm64":
  scons {{compile_commands}} -j4 gdextension_dir={{api_dir}} platform=android arch=arm64

build-android-x86_32 $SCONS_CACHE="build-android-x86_32":
  scons {{compile_commands}} -j4 gdextension_dir={{api_dir}} platform=android arch=x86_32
build-android-x86_64 $SCONS_CACHE="build-android-x86_64":
  scons {{compile_commands}} -j4 gdextension_dir={{api_dir}} platform=android arch=x86_64

build-windows-x86_32 $SCONS_CACHE="build-windows-x86_32":
  scons {{compile_commands}} -j4 gdextension_dir={{api_dir}} platform=windows arch=x86_32
build-windows-x86_64 $SCONS_CACHE="build-windows-x86_64":
  scons {{compile_commands}} -j4 gdextension_dir={{api_dir}} platform=windows arch=x86_64
